...
# Get value for some user from log
#sub radius_log  {
# my $uid = shift;
# my $grep = '';
#
# if ($uid ne "") {
#    $grep  = '/usr/bin/grep -E "\['. $uid . '\W*(\/|\])" '. $RADLOG;
#   }
# else {
#    my $date = `date "+%b %d"`;
#    $grep  = '/usr/bin/grep "Auth.notice: Login incorrect:" '. $RADLOG;
#   }

# Apr 21 13:13:35: Auth: Login OK: [kolon]: CLID unknown (from nas NETServer)
#open(FILE, "$grep \| tail -$rad_rows|") || die "Can't open file $RADLOG $!\n";
#  while(<FILE>) {
#     print "-$_";
#   } 
#close(FILE);
